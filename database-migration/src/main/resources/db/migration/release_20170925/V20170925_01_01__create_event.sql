...
CREATE TABLE IF NOT EXISTS `studio_players` (
  ...
  COMMENT 'プレイヤーID',
  studio_player_path              VARCHAR(255) NOT NULL
  COMMENT 'プレイヤーページパス',
  studio_player_name_prefix       VARCHAR(255) COMMENT 'プレイヤー名称接頭',
  studio_player_name_postfix      VARCHAR(255) COMMENT 'プレイヤー名称接尾',
  studio_player_short_description TEXT         NOT NULL
  COMMENT 'プレイヤー短詳細',
  studio_player_description       TEXT         NOT NULL
  COMMENT 'プレイヤー詳細',
...
CREATE TABLE IF NOT EXISTS `studio_player_members` (
  ...
  studio_player_member_name VARCHAR(255) NOT NULL
  COMMENT 'プレイヤーメンバー名称',
  display_order_number      INTEGER      NOT NULL
  COMMENT '表示順',
  created_at                DATETIME     NOT NULL
...
CREATE TABLE IF NOT EXISTS `studio_player_x_members` (
  studio_player_id        VARCHAR(26) NOT NULL
  COMMENT 'プレイヤーID',
  studio_player_member_id VARCHAR(26) COMMENT 'スタジオプレイヤーID',
  created_at              DATETIME    NOT NULL
  COMMENT 'データ作成日時',
...
CREATE TABLE IF NOT EXISTS `events` (
  ...
  COMMENT 'イベント詳細',
  event_open_datetime  DATETIME     NOT NULL
  COMMENT 'イベント開場日時',
